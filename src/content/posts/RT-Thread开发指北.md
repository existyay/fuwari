...
STM32CubeMX 其实是依靠着java8的后端逻辑和对应的前端页面进行设置其对应的硬件资源，其需要在移植正确RT-Thread对应的资源之后才可以使用其资源,对应的板层资源的头文件一般会在CubeMX_Config\Inc下 对应的

# 移植和准备开发 

* 参考RT-Thread的nano文档但是书写和使用写的是标准版
...
板级支持包，厂商提供的BSP，让这款芯片能在特定环境下运行下去 使用的是RT-Thread 需要STM32F103ZET6的通用源码然后制作对应的STM32F103ZET6的BSP
...
RT-Thread存在两类线程、分别为用户线程和系统线程、系统线程是由 RT-Thread 内核创建的线程，用户线程是由应用程序创建的线程，这两类线程都会从内核对象容器中分配线程对象，当线程被删除时，也会被从对象容器中删除，每个线程都有重要的属性，如**线程控制块、线程栈、入口函数**等。RT-Thread 的线程调度器是**抢占式**的，主要的工作就是从**就绪线程列表**中查找最高优先级线程，保证最高优先级的线程能够被运行，最高优先级的任务一旦就绪，总能得到 CPU 的使用权。当一个运行着的线程使一个比它优先级高的线程满足运行条件，当前线程的 CPU 使用权就被剥夺了，或者说被让出了，高优先级的线程立刻得到了 CPU 的使用权。如果是中断服务程序使一个高优先级的线程满足运行条件，中断完成时，被中断的线程挂起，优先级高的线程开始运行。当调度器调度线程切换时，先将当前线程上下文保存起来，当再切回到这个线程时，线程调度器将该线程的上下文信息恢复
...
| :--------------- | :---------------------------------- | :--------------------------- | :----------- | :----------------- | :----------------- | :------------------------------------------------ | :--------------------------------------------- |
| **UART**   | **异步串行通信协议** (逻辑层) | 2 (TX, RX)                   | 全双工，异步 | 9600 bps ~ 几 Mbps | 短                 |                                                   | 设备调试、GPS、蓝牙模块通信                    |
| **I2C**    | **同步串行总线** (板载)       | 2 (SDA, SCL)                 | 半双工，同步 | 多主多从           | 1-2米 (板内)       | 标准模式100kbps，高速模式400kbps，最高可达3.4Mbps | 连接板载传感器、EEPROM、RTC等低速外设          |
| **SPI**    | **同步串行接口** (板载)       | 3+n (SCLK, MOSI, MISO, n*CS) | 全双工，同步 | 主从通信           | 1-2米 (板内)       | 通常1-70 Mbps，可更高                             | Flash存储器、高速ADC/DAC、显示屏               |
...
ADC模数转换器、单元，将模拟量转化为数字量，因为单片机能够直接处理的是数字信号，但是生活中很多传感器采集到的是模拟量，比如—>光照、声音、气体浓度、电压、电流、所以需要ADC来进行辅助处理(需要转换成更容易储存、处理和发射的数字形式)，与之相对应的 DAC(Digital-to-Analog Converter)，它是 ADC 模数转换的逆向过程，ADC 最早用于对无线信号向数字信号转换。如电视信号，长短播电台发射接收等。
12位ADC是一种逐次逼近型模拟数字转换器。它有多达18个通道，可测量16个外部和2个内部
信号源。各通道的A/D转换可以单次、连续、扫描或间断模式执行。ADC的结果可以左对齐或右
对齐方式存储在16位数据寄存器中。
模拟看门狗特性允许应用程序检测输入电压是否超出用户定义的高/低阀值。
ADC的输入时钟不得超过14MHz，它是由PCLK2经分频产生。
12位分辨率 及其转换范围是0~4095
...
|                    | 转换范围     | 0 ~ 4095 (2¹² = 4096)           |
| **数据对齐** | 左对齐       | 数据向左对齐存储                  |
|                    | 右对齐       | 数据向右对齐存储（推荐/一般配置） |
| **时钟配置** | 输入时钟频率 | < 14 MHz（最大限制）              |
|                    | 推荐工作频率 | 根据具体应用配置                  |
| **其他参数** | 参考电压     | 需根据具体芯片规格确定            |
|                    | 采样时间     | 需根据输入信号特性配置            |
...
![ADC 转换过程](https://www.rt-thread.org/document/site/rt-thread-version/rt-thread-standard/programming-manual/device/adc/figures/adc-p.png)
...
光敏电阻本身也是一个阻值可变的电阻，阻值改变会影响电压的改变,ADC转换量也会改变->反映光照的强弱，光照越强->光敏电阻阻值越小 电压越小
...
模块介绍 MY1680 只能播报 不能识别 要将想要播报的语音和对应的音乐复制上去支持mp3和wav文件，MY1680-12P 是深圳市千佰度电子科技有限公司自主研发的一款小巧的微集成MP3模块。采用MY1680 SOP16 MP3主控芯片，支持MP3、WAV格式双解码。模块内置FLASH存储芯片，1-16M容量可选；也可外接 U盘或USB数据线连接电脑更换FLASH的音频文件。该模块内置3W功放，可以直接驱动3W的喇叭，使用更方便。24 位 DAC 输出，动态范围支持 93dB，信噪比支持 85dB，完全支持 FAT16、FAT32 文件系统，最大支持 16M FLASH，32G 的 U 盘。支持 UART 异步串口控制：支持播放、暂停、上下曲、音量加减、选曲播放、插播等。ADKEY 功能，通过电阻选择可实现标准 MP3 功能的 5 按键控制和其他功能。内置音量、曲目掉电记忆功能。采样率8～48K、比特率8～320Kbps 音频文件，标准串口，3.3V TTL 电平,波特率9600，接3W/4Ω或1W/8Ω喇叭 ，MY1680-12P内置标准UART异步串口接口，为3.3V TTL电平接口。可通过MAX3232芯片转换成RS232电平或者通   过 USB转TTL模块与PC通讯进行调试。通讯数据格式是：起始位：1位；数据位：8位；奇偶位：无；停止位：1位。还是1801
...
| :----- | :----------------------------------------------------- | :----- |
| 起始码 | 命令开始标志，固定值                                   | 7E     |
| 长度   | 从“长度”字段到“校验码”字段的字节总数（十六进制）   | 04     |
| 操作码 | 指令代码（十六进制）                                   | 31     |
| 参数   | 可选参数，可能没有或为两个字节（十六进制）             | 19     |
| 校验码 | “长度”、“操作码”、“参数”依次异或的值（十六进制） | 2C     |
| 结束码 | 命令结束标志，固定值                                   | EF     |

**注意：**

- “长度”字段的计算：统计从“长度”到“校验码”的所有字节个数（十六进制表示）。例如，示例中长度值为 `04`，表示有 `04`、`31`、`19`、`2C` 共 4 个字节。
- “校验码”的计算：依次对“长度”、“操作码”、“参数”进行**异或**运算。例如，`04 xor 31 xor 19 = 2C`。
- 示例指令：设置音量指令为 `7E 04 31 19 2C EF`。
- 长度 = 长度 操作码 校验码 结束码 （起始码是固定的）

# RFID

## 什么是RFID

无线射频识别是通过无线电波实现读卡器和电子标签(应答器)的非接触式自动识别技术，可精准识别目标，广泛应用于动物识别、汽车防盗、门禁管理、生产线自动化及物料追踪等领域。一个完整的RFID系统由读写器、应答器、软件系统组成
...
每个扇区的块3为**控制块**，包括了密码A、存取控制、密码B。

密码A（6字节） 存取控制（4字节） 密码B（6字节）

每个扇区的密码和存取控制都是独立的，可以根据实际需要设定各自的密码及存取控制。存取控制为4个字节，共32位，扇区中的每个块（包括数据块和控制块）的存取条件是由密码和存取控制共同决定的，在**存取控制**中每个块都有相应的**三个控制位**
...
MCU和其模块使用SPI通信 简单了解一下SPI通信

# SPI通信

在硬件层SPI通信有四根通信线：SCK（时钟线），MOSI（主机输出从机输入数据线），MISO（主机输入从机输出数据线），CS/SS（片选信号线）

MOSI 主机输出 从机输入 主机->从机

MISO 从机输出 主机输入 从机->主机

SCK时钟线

CS/SS 片选线 空闲时片选线为高电平 下降沿开始工作，主机与从机通信的时候要拉低

极性和相位

相位 从时钟线的第几个边沿(上升沿和下降沿)开始采样

0:从第一个边沿

1:从第二个边沿

0模式-----极性0，相位0 时钟线空闲时低电平，第一个沿（上升沿）开始采样

等字节交换(高位先行原则)

主机高位先发数据到从机低位通过MOSI、从机高位先发数据到主机的低位 通过MISO

# WIFI

wifi是一种常见的无线通信方式，它的工作原理是将宽带网线接入到路由器，路由器将有线信号调制成2.4Ghz或者5Ghz频段的射频信号 向外广播无线热点，终端设备去连接热点实现上网

WIFI芯片 ESP8266 WIFI模组ESP-12F

采用SMD-22封装 2400Mhz~2483.5Mhz 2.4Ghz频段

3.0V到3.6V 默认115200bps

安全性 WEP/WPA-PSK/WPA2-PSK

默认32Mbit

软件协议: AT指令集开发，厂家已经烧录好对应的AT固件 不同的AT指令对应不同的功能

2.2.1 功能说明  ATK-ESP8266 模块支持STA/AP/STA+AP 三种工作模式。

* STA 模式：ESP8266模块通过路由器连接互联网，手机或电脑通过互联网实现对设备的远程控制。(站点模式)
* AP 模式：默认模式ATK_ESP8266模块作为热点，实现手机或电脑直接与模块通信，实现局域网无线控制。 (接入点模式)
* STA+AP 模式：两种模式的共存模式(混合模式)，（STA模式）即可以通过路由器连接到 互联网，并通过互联网控制设备；（AP模式）也可作为wifi热点，其他wifi 设备连接到模块。这样实现局域网和广域网的无缝切换，方便操作。
  | 命令类型           | 指令格式           | 功能描述                                                 |
  | :----------------- | :----------------- | :------------------------------------------------------- |
  | **测试命令** | `AT+<CMD>=?`     | 该命令用于查询设置命令或内部程序设置的参数以及其他范围。 |
  | **查询命令** | `AT+<CMD>?`      | 该命令用于返回参数的当前值。                             |
  | **设置命令** | `AT+<CMD>=<...>` | 该命令用于设置用户自定义的参数值。                       |
  | **执行命令** | `AT+<CMD>`       | 该命令用于执行受模块内部程序控制的变参数不可变的功能。   |

基础AT测试模块是否有问题、没有问题之后直接连接WIFI

设置对应的工作模式给定wifi名称和密码

AT+CWMODE= `<mode>` 1:Station 2:AP 3:AP+Station

AT+CWJAP = `<ssid>`,`<password>`

调试打印->USART1 语音播报->USART2 WIFI->USART3 TX PB10 RX PB11
